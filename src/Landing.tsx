...
export default function Landing() {
    ...
    const bind = useDrag(({ event, down, movement }) => {
        ...
        set2.start({ xy: down ? movement : [0, 0] });
    }, {
        rubberband: true, 
    ...
    const calc = (o: number) => { return `translateY(${o * 0.1}px)`; };
    ...
    return (
        <animated.div className={classes.root} style={{ transform: offset.to(calc) }}>
            <animated.div {...bind()} className={classes.logo} style={{ transform: xy.to((x: number, y: number) => `translate3d(${x}px, ${y}px, 0)`) }} />
